...
class Stripe{
	...
	public function getPaymentIntent($customers_id, $price){
        ...
        if($customers_id>0){
            $customersObj = $this->db->getObj("SELECT first_name, email, contact_no, branches_id FROM customers WHERE customers_id = $customers_id", array());
            if($customersObj){
		...
		if($posObj){
			...
			if ($httpCode === 200) {
				...
				if (isset($chargeData['id'])) {
					...
					$amount = round($chargeData['amount']/100,2);

					$totalpayment = 0;
					$ppSql = "SELECT payment_method, payment_amount, pos_payment_id FROM pos_payment WHERE pos_id = $pos_id AND payment_method != 'Change'";
					$ppQueryObj = $this->db->getObj($ppSql, array());
					if($ppQueryObj){
						while($onerow = $ppQueryObj->fetch(PDO::FETCH_OBJ)){
							
							$totalpayment = round($onerow->payment_amount,2);
							if($totalpayment != $amount && $p==0 && $onerow->payment_method=='Stripe'){
								$p++;
								$amount = $amount-$totalpayment;
								//$this->db->update('pos_payment', array('payment_amount'=>$amount), $onerow->pos_payment_id);

								$stripeChargeUrl = "https://api.stripe.com/v1/refunds";
