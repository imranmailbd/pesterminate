...
$code = substr($segment3name, 0, 20);
if($code !=''){
	if($segment4name=='' || $segment4name==0){$segment4name = 40;}
	$height = intval($segment4name);
	
	$code = str_replace('_and_', '&', $code);
	$code = str_replace('_plus_', '+', $code);
	$code = str_replace('_bksls_', '/', $code);
	$density = 1;
