...
class News_articles{
	...
	public function lists($segment3){
		...
		<div class=\"row\">
			<div class=\"col-sm-12 col-md-6\">
				<h1 class=\"metatitle\">$GLOBALS[title] <i class=\"fa fa-info-circle txt16normal\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"This page displays the list of your News & Articles\"></i></h1>
			</div>	
			<div class=\"col-sm-12 col-md-6 ptopbottom15\">
				<a href=\"javascript:void(0);\" title=\"Create News & Articles\" onClick=\"AJget_News_articlesPopup(0);\">
					<button class=\"btn btn-default hilightbutton floatright\"><i class=\"fa fa-plus\"></i> <strong>New News & Articles</strong></button>
				</a>
			</div>  
		</div>
		<div class=\"row\">
			...
			<div class=\"col-sm-12 col-sm-4 col-md-3 pbottom10\">
				<div class=\"input-group\">
					<input type=\"text\" placeholder=\"Search News & Articles\" value=\"$keyword_search\" id=\"keyword_search\" name=\"keyword_search\" class=\"form-control\" maxlength=\"50\" />
					<span class=\"input-group-addon cursor\" onClick=\"checkAndLoadFilterData();\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Search News & Articles\">
   ...
   private function loadTableRows(){
		...
		if($query){
			...
			$str .= "<tr><td colspan=\"4\" class=\"red18bold\">No News & Articles meet the criteria given</td></tr>";
		}
		return $str;
   }
	...
	public function view($segment3, $segment4){
		...
		if($news_articlesObj){
			...
			$htmlStr = "<div class=\"row\">
				<div class=\"col-sm-8\">
					<h1 class=\"metatitle\">$GLOBALS[title] <i class=\"fa fa-info-circle txt16normal\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"This page displays the information of News & Articles\"></i></h1>
				</div>
				<div class=\"col-sm-4 ptopbottom15\">
					<a href=\"/News_articles/lists\" title=\"News & Articles List\">
						<button class=\"btn btn-default hilightbutton floatright\"><i class=\"fa fa-list\"></i> <strong>News & Articles List</strong></button>
					</a>
				</div>    
			</div>";
	...
	private function filterHAndOptions(){
		...
		if($shistory_type !=''){
			if(strcmp($shistory_type, 'news_articles')==0){
				$filterSql = "SELECT 'News & Articles Created' AS afTitle, 'news_articles' AS tableName FROM news_articles WHERE news_articles_id = :news_articles_id";
			}
			...
			UNION ALL SELECT 'News & Articles Created' AS afTitle, 'news_articles' AS tableName FROM news_articles WHERE news_articles_id = :news_articles_id 
   ...
   private function loadHTableRows(){
		...
		if($shistory_type !=''){
			if(strcmp($shistory_type, 'news_articles')==0){
				$filterSql = "SELECT users_id, created_on, 'News & Articles Created' AS afTitle, 'news_articles' AS tableName, news_articles_id AS tableId FROM news_articles WHERE news_articles_id = :news_articles_id";
			}
			elseif(strcmp($shistory_type, 'notes')==0){
			...
			UNION ALL SELECT users_id, created_on, 'News & Articles Created' AS afTitle, 'news_articles' AS tableName, news_articles_id AS tableId FROM news_articles WHERE news_articles_id = :news_articles_id 
		...
		if($news_articles_id==0){
			...
			if($totalrows>0){
				...
				if($news_articles_id){
					...
					$savemsg = 'error';
					$message .= "<p>'Error occured while adding new News & Articles! Please try again.'</p>";
				}
			}
		}
		else{
